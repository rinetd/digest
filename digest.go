...
func Get(rawurl string) (resp *http.Response, err error) {
	...
	DigestClient.Username = u.User.Username()
	DigestClient.Password, _ = u.User.Password()

	request, err := http.NewRequest("GET", rawurl, nil)
	if err != nil {
		// return err
	}
	return DigestClient.RoundTrip(request)
...
func parseChallenge(input string) (*challenge, error) {
	...
	for i := range sl {
		...
		case "qop":
			...
			if c.Qop == strings.ToLower("auth-int") {
